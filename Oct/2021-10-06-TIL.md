...
### ◆ ***ForeignKey***
Foreignkey는 다대일 관계를 나타내고 기본 형태는 `Foreignkey(to, on_delete, **options)`이다.

재귀적 관계(자식과 다대일 관계가 있는 객체)를 만들려면 

`models.ForeignKey('self', on_delete=models.CASCADE)`를 사용하면 된다. → 21.10.04TIL

아직 정의되지 않은 모델에 대한 관계를 생성해야 하는 경우에는 모델 객체 자체가 아닌 

'모델 이름'을 사용할 수 있다. 예를 들면
```python
...
에러 메세지에서는 instagra.Post.author 와 like_user_set의 `reverse name`이 충돌했으니 

수정하라고 하는 것이다. 즉, ForeignKey에서 `"모델명_set"`이 겹쳐서 에러가 발생했다는 뜻.

따라서 django에서의 'ForeignKey.related_name'에 대해서 자세히 공부하기로 했다.

Django Documents를 읽어보니 <span style="color:green">**Following relationships “backward”**</span>을 읽어보라고 했다.


아무튼, `related_name`은 관련 객체에서 다시 이 객체로의 관계에 사용할 이름이다. 

또한 related_query_name(대상의 역 필터 이름에 사용할 이름)의 기본값이기도 하다. 

Django가 역방향 관계를 생성하지 않도록 하려면 related_name을 '+'로 설정하거나 
